...
import { primeAppVersion } from "@/lib/app/appVersion";

// Ensure settings (theme, font, etc) are initialized immediately
initSettings();

// Prime the app-version cache as early as possible so all services that read
// it synchronously (telemetryCollector, perfLogService) see a real value.
...
window.addEventListener(
  ...
  (event: Event) => {
    ...
    console.warn(
      "[Clypra] WebGL context lost — GPU reset or driver TDR detected.",
    );
    void crashReporter.reportCrash({
      ...
      error: {
        ...
        message:
          "GPU/WebGL context was lost. A driver reset or resource exhaustion may have occurred.",
      },
