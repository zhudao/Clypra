...
export const EffectCard: React.FC<EffectCardProps> = ({
  ...
  return (
    <div
      onClick={onPreview}
      className="w-full aspect-square bg-surface-raised/40 hover:bg-surface-raised/80 border border-border/40 hover:border-accent/40 rounded-xl relative overflow-hidden flex flex-col justify-between p-2.5 transition-all duration-300 group cursor-pointer shadow-[0_4px_16px_rgba(0,0,0,0.3)]"
    >
      {/* Downloading Overlay */}
      {isDownloading && (
      ...
      <button
        onClick={onFavorite}
        className={`absolute top-2 right-2 p-1 cursor-pointer rounded-full bg-surface/40 hover:bg-surface/60 border border-border/50 text-text-muted hover:text-text-primary transition-all duration-200 z-10 ${
      ...
      <div className="flex-1 flex items-center justify-center w-full px-1 py-3 select-none relative overflow-hidden transition-transform duration-500 ease-out group-hover:scale-[1.05]">
        {thumbnailUrl ? (
          <img
            src={thumbnailUrl}
            alt={effect.name}
      ...
      <div className="flex items-center justify-between w-full mt-0.5 z-10">
        ...
        >
          {isDownloading ? (
            ...
            <Download className="w-2.5 h-2.5 group-hover:scale-115 transition-transform" />
          )}
        </button>
      </div>
    </div>
