...
import { Plus, Trash2, AlignLeft, AlignCenter, AlignRight, Type, ShieldAlert, User } from "lucide-react";
    ...
    <div className="flex flex-col gap-3">
      ...
      {selectedLayer ? (
        <div className="flex flex-col gap-3 p-3 bg-neutral-900/70 rounded-xl border border-neutral-800">
          ...
          {/* Depth Placement: Behind Person vs In Front */}
          <div className="flex items-center justify-between p-2 rounded-lg bg-neutral-800/70 border border-neutral-700 select-none">
            <div className="flex items-center gap-2">
              <div className={`p-1 rounded ${selectedLayer.behindSubject ? "bg-purple-500/20 text-purple-400" : "bg-neutral-700 text-neutral-400"}`}>
                <User className="w-3.5 h-3.5" />
              </div>
              <div className="flex flex-col">
                <span className="text-[11px] font-semibold text-neutral-200">Behind Subject</span>
                <span className="text-[9px] text-neutral-400">Place layer behind AI subject cutout</span>
              </div>
            </div>
            <button
              type="button"
              onClick={() => onUpdateLayer(selectedLayer.id, { behindSubject: !selectedLayer.behindSubject })}
              className={`px-2.5 py-1 rounded text-[10px] font-bold border transition-colors ${
                selectedLayer.behindSubject
                  ? "bg-purple-500/25 border-purple-500 text-purple-300 shadow-sm"
                  : "bg-neutral-800 border-neutral-700 text-neutral-400 hover:text-neutral-300"
              }`}
            >
              {selectedLayer.behindSubject ? "Behind Person" : "In Front"}
            </button>
          </div>
