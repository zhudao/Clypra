import {
  ...
  TIMELINE_MIN_CANVAS_DURATION_SECONDS,
  TIMELINE_CANVAS_PADDING_SECONDS,
} from "./timelineClip";
import {
...
/** Deliberate breathing room between the sticky track labels and t=0. */
export const TIMELINE_CLIP_START_OFFSET_PX = 20;

export function getTimelineLabelColumnWidth(hasClips: boolean): number {
  return hasClips ? TIMELINE_TRACK_LABEL_WIDTH_PX : 0;
}

export function getTimelineLaneWidth(
...
export function getTimelineLaneClientX(
  clientX: number,
  ...
  return Math.max(
    0,
    clientX - containerLeft - getTimelineLabelColumnWidth(hasClips),
  );
}

export function getTimelineScrollContentWidth(
  ...
  return (
    ...
    (hasClips ? TIMELINE_CLIP_START_OFFSET_PX : 0) +
    Math.round(viewportEndSeconds * pixelsPerSecond)
  );
}
...
export function zoomToPixelsPerSecond(zoomLevel: number): number {
  return clampTimelinePixelsPerSecond(
    TIMELINE_PPS_PER_ZOOM * clampTimelineZoom(zoomLevel),
  );
}
...
export function getAnchoredZoomScrollLeft(input: {
  ...
  const nextScrollLeft =
    timelineTimeToPixel(input.anchorTime, input.nextPixelsPerSecond) -
    input.localTimelineX;
  return clampTimelineScrollLeft(
    nextScrollLeft,
    input.containerWidth,
    input.viewportEndSeconds,
    input.nextPixelsPerSecond,
    input.hasClips,
  );
}

export function getTimelineViewportEndForDuration(
...
export function timeToPixel(
  ...
  const validPPS =
    ...
    pixelsPerSecond > 0
      ...
      : 50;
  const validTime =
    typeof timeInSeconds === "number" && !isNaN(timeInSeconds)
      ? timeInSeconds
      : 0;
...
export function pixelToTime(
  ...
  const validPPS =
    ...
    pixelsPerSecond > 0
      ...
      : 50;
  const validPixel =
    typeof pixelOffset === "number" && !isNaN(pixelOffset) ? pixelOffset : 0;
  return validPixel / validPPS;
}

/** Compute the density needed to show the entire sequence in the usable lane. */
export function getFitSequencePixelsPerSecond(
  containerWidth: number,
  duration: number,
  hasClips: boolean,
): number {
  if (duration <= 0) return zoomToPixelsPerSecond(1);
  const usableLaneWidth = Math.max(
    1,
    getTimelineLaneWidth(containerWidth, hasClips) -
      (hasClips ? TIMELINE_CLIP_START_OFFSET_PX : 0),
  );
  return clampTimelinePixelsPerSecond(
    usableLaneWidth / duration,
    true,
  );
}
...
export function getScrollLeftToRevealTime(input: {
  ...
  const editPointPixels = timelineTimeToPixel(
    input.time,
  ...
  if (editPointPixels > visibleRight)
    next = editPointPixels - laneWidth + inset;
  return clampTimelineScrollLeft(
    next,
    input.containerWidth,
    input.viewportEndSeconds,
...
/** Convert a timeline time to a lane-local rendered pixel coordinate. */
export function timelineTimeToPixel(
  timeInSeconds: number,
  pixelsPerSecond: number,
): number {
  return timeToPixel(timeInSeconds, pixelsPerSecond) + TIMELINE_CLIP_START_OFFSET_PX;
}

/** Convert a lane-local rendered pixel coordinate back to timeline time. */
export function timelinePixelToTime(
  pixelOffset: number,
  pixelsPerSecond: number,
): number {
  return pixelToTime(
    pixelOffset - TIMELINE_CLIP_START_OFFSET_PX,
    pixelsPerSecond,
  );
}

/** Convert a pointer coordinate to scrollable, time-origin-relative lane px. */
export function getTimelineLaneContentX(
  clientX: number,
  containerLeft: number,
  scrollLeft: number,
  hasClips: boolean,
  ...
  return (
    getTimelineLaneClientX(clientX, containerLeft, hasClips) +
    scrollLeft -
